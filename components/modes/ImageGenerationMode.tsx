        ...
        <div className="flex flex-col h-full w-full p-6 md:p-8 animate-subtle-fade-in-up">
            ...
            <div className="flex-1 grid md:grid-cols-2 gap-8 overflow-hidden">
                <div className="flex flex-col space-y-6">
                    ...
                    >
                        {isLoading ? <ThematicLoader type="default" /> : <Wand2 className="mr-2" />}
                        {isLoading ? 'Forging...' : 'Generate'}
                    </button>
                    {error && <p className="text-red-400 text-center">{error}</p>}
                </div>
                
                <div className={`w-full h-full ${bgColor} rounded-xl flex items-center justify-center relative overflow-hidden border-2 border-dashed border-white/20 ${isLoading ? 'generating-glow' : ''}`}>
                    {isLoading && (
                        <div className="flex flex-col items-center text-white/50 z-10">
                           <ThematicLoader type="image" accentColor={accentColor} />
                            <p className="mt-4">Forging your vision...</p>
                        </div>
                    )}
                    {!isLoading && !generatedImage && <ImageIcon className="w-24 h-24 text-white/20" />}
                    {generatedImage && (
                        <>
                            <img src={generatedImage} alt="Generated art" className="object-contain w-full h-full animate-subtle-fade-in z-10" />
                            <a href={generatedImage} download="nihara-art.jpg" className={`absolute bottom-4 right-4 p-3 rounded-full bg-${accentColor} text-white hover:scale-110 transition-transform duration-200 interactive-glow z-10`}>
                                <Download />
                            </a>
                        </>
                    )}
