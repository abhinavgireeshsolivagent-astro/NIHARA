...
const TypingIndicator: React.FC<{ isPro: boolean }> = ({ isPro }) => (
    <div className={`flex items-center space-x-2 h-10 px-4 rounded-2xl ${isPro ? 'bg-emerald-900/50' : 'bg-slate-700'}`}>
...
const ChatMode: React.FC<ChatModeProps> = ({ userName, personality, chatHistory, setChatHistory, onInteraction, isPro }) => {
    ...
    const proInputClass = "border-emerald-500/50";
    ...
    const renderMessage = (msg: ChatMessage, index: number, messageId: string) => {
        ...
        const userBubbleStyles = `rounded-tr-lg chat-bubble-user-lively`;
        const aiBubbleStyles = `rounded-tl-lg ${isPro ? "chat-bubble-ai-pro-lively" : "bg-slate-700"}`;
